...
std::ostream& operator<< (std::ostream& out, const solver::result& r) {
    ...
    std::ofstream csvout(filename, std::ios::app);
    if (!csvout) {
    ...
    if (!file_exists) {
        csvout << "Solution Type,States Searched,Unique States Searched,Backtracks,Dominance Moves,"
               "States Removed From Cache,Final States In Cache,Final Buckets In Cache,"
               "Maximum Search Depth,Final Search Depth,Time Taken (milliseconds)\n";
    }

    csvout << r.sol_type << ","
        << r.states_searched << ","
        << r.unique_states_searched << ","
        << r.backtracks << ","
        << r.dominance_moves << ","
        << r.states_removed_from_cache << ","
        << r.cache_size << ","
        << r.cache_bucket_count << ","
        << r.max_depth << ","
        << r.depth << ","
        << r.time.count() << "\n";
