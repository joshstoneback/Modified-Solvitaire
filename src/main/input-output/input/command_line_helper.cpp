...
command_line_helper::command_line_helper()
    ...
    hidden_options.add_options()
            ("input-files", po::value<vector<string>>(),
             "the file(s) containing the deal to be solved")
            ("input-folder", po::value<string>(), 
            "the folder containing the deals to be solved")
             ;
...
bool command_line_helper::parse(int argc, const char* argv[]) {
    ...
    if (vm.count("input-folder")){
        input_folder = vm["input-folder"].as<string>();
    } else {
        char cwd[PATH_MAX];  
    ...
    if (vm.count("input-files")) {
        ...
        if (!dir) {
            std::cerr << "Error: Cannot open folder " << input_folder << "\n";
        }
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            if (filename != "." && filename != "..") {
                input_files.push_back(input_folder + "/" + filename);
            }
        }

        closedir(dir);
        cerr << "Auto-selecting input files as all files in the selected folder" << endl;
    }

    if (vm.count("type")) {
        solitaire_type = vm["type"].as<string>();
    } else {
        solitaire_type = "klondike";
        cerr << "Auto-selecting klondike as solitaire type" << endl;
    }
...
const string command_line_helper::get_input_folder(){
    return input_folder;
}
